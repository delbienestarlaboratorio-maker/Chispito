...
export default async function GradoPage({ params }: Props) {
    try {
        ...
        return (
            <div className="min-h-screen bg-red-900 text-white p-10 font-mono">
                <h1 className="text-4xl font-bold mb-4">EXCEPCIÓN FATAL (EDGE WORKER)</h1>
                <p className="text-xl mb-4 font-bold bg-black p-4 inline-block">{edgeError.message}</p>
                <pre className="bg-black/80 p-6 rounded-xl overflow-auto text-sm">
                    {edgeError.stack || "No stack trace disponible"}
                </pre>
            </div>
        );
    }
